...
const locationMapper_1 = require("../utils/locationMapper");
...
function getMapHelperCalleeName(callee) {
...
function extractNamesFromMapHelperArg(arg) {
...
function addSymbol(list, name, type, line, column, blockIndex, source) {
    if (name && typeof line === 'number' && typeof column === 'number') {
        list.push({ name, type, line, column, blockIndex, source });
    }
}
function toSymbolNodes(blocks, rawList) {
    return rawList.map(r => {
        const block = blocks[r.blockIndex] ?? blocks[0];
        const loc = (0, locationMapper_1.mapToFileLocation)(block.startLine, block.startColumn, r.line, r.column);
 ...
 */
function analyzeScriptContent(content, isSetup, isTypeScript, blockIndex, out) {
    let ast;
    try {
    ...
    const push = (name, type, line, col, source) => addSymbol(out, name, type, line, col, blockIndex, source);
    ...
    (0, traverse_1.default)(ast, {
            ...
            for (const s of specifiers) {
                if (s.type === 'ImportDefaultSpecifier' || s.type === 'ImportSpecifier') {
                    const name = s.type === 'ImportDefaultSpecifier'
                        ? s.local.name
                        : (s.imported.type === 'Identifier' ? s.imported.name : s.imported?.value ?? s.local.name);
        ...
        ExportDefaultDeclaration(path) {
            ...
            const getLineCol = (node) => node.loc ? [node.loc.start.line, node.loc.start.column] : [1, 0];
            for (const prop of obj.properties) {
                ...
                if (typeof key !== 'string')
                    continue;
                const [line, col] = getLineCol(prop);
                if (key === 'components' && prop.type === 'ObjectProperty' && prop.value.type === 'ObjectExpression') {
                        ...
                        if (val.type === 'ArrayExpression') {
                            for (const el of val.elements) {
                                if (el && el.type === 'StringLiteral')
                                    push(el.value, 'prop', el.loc?.start.line ?? line, el.loc?.start.column ?? col);
                                if (el && el.type === 'Identifier')
                                    push(el.name, 'prop', el.loc?.start.line ?? line, el.loc?.start.column ?? col);
                            }
                        }
                    ...
                    if (prop.type === 'ObjectProperty' && prop.value.type === 'ArrayExpression') {
                        for (const el of prop.value.elements) {
                            if (el && el.type === 'StringLiteral')
                                push(el.value, 'emit', el.loc?.start.line ?? line, el.loc?.start.column ?? col);
                            if (el && el.type === 'Identifier')
                                push(el.name, 'emit', el.loc?.start.line ?? line, el.loc?.start.column ?? col);
                        }
                    }
                }
                    ...
                    for (const p of prop.value.properties) {
                        if (p.type === 'ObjectProperty') {
                            ...
                            const calleeName = getMapHelperCalleeName(p.argument.callee);
        ...
        ...(isSetup ? {
            ...
            VariableDeclarator(path) {
                ...
                if (init?.type === 'CallExpression') {
                    ...
                    if (['ref', 'reactive', 'computed', 'watch', 'watchEffect'].includes(name))
                        return;
                }
...
function analyzeSetupFunctionBody(body, out, blockIndex) {
    const add = (name, type, line, col) => addSymbol(out, name, type, line, col, blockIndex);
